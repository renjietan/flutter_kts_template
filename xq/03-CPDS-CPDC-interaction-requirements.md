...
- 除发现设备数量不匹配确认框中的“取消”外，不支持用户暂停、取消或手动终止。
...
stateDiagram-v2
    ...
    Discovering --> AwaitingDiscoveryConfirmation: 5秒窗口结束、仅数量不匹配且存在可匹配设备
    Discovering --> Failed: ESN冲突、发现报文无效或无可匹配设备
    AwaitingDiscoveryConfirmation --> Authenticating: 用户继续且存在可匹配设备
    AwaitingDiscoveryConfirmation --> Failed: 用户取消
    ...
    Parsing --> Completed: 数量原本匹配且全部设备解析成功并已发送ACK
    Parsing --> PartialSuccess: 数量不匹配后继续且全部目标进入终态、至少一台成功
    Parsing --> DrainingAfterFailure: 任一设备解析失败或30秒超时
    DrainingAfterFailure --> PartialSuccess: 至少一台成功且其余设备进入终态
    DrainingAfterFailure --> Failed: 全部目标设备均失败
    Completed --> Ready
    PartialSuccess --> Ready
    Failed --> Ready
```
...
- 类型集合和各类型数量完全相同：发现成功，直接进入认证。
- 仅存在缺少、多出或各类型数量不一致：进入数量不匹配确认流程。
- ESN冲突、非法设备类型或发现报文无效：发现失败，不允许继续。

第5秒结束后统一比较窗口内全部有效回复。数量不匹配且至少存在一台可匹配在线设备时，CPDS进入等待用户确认状态且不得发送 `AUTH_NTY`；弹窗提示“设备数量与发现设备数量不匹配，是否继续下发？”，并按类型显示期望、发现、缺少和多出数量，例如“缺少1台MultiBandRadio”或“多出1个Server”。

存在至少一台可匹配在线设备时，弹窗提供“继续”和“取消”。用户选择“继续”后，CPDS按第6.1节规则冻结只包含已匹配在线设备的认证目标清单，多出的发现设备不生成认证记录、不接收文件并从本次下发中忽略；用户选择“取消”时不进入认证，本次任务完成并标记为失败。没有任何可匹配在线设备时不显示数量确认框，直接把任务标记为失败并显示一次最终失败弹窗。

会话按成功设备数量和发现差异统一结算：没有任何设备成功时标记为失败；至少一台成功，但存在未发现、被忽略或失败设备时标记为部分成功；数量完全匹配且全部设备成功时才标记为全部成功。数量不匹配后继续的会话即使精简目标清单内全部设备成功也仍为部分成功。该确认和结果语义属于CPDS本地状态，不增加或修改UDP协议消息。
...
3. 只在两个列表的共同索引范围内配对ESN与设备ID；缺少的期望设备保持未发现，多出的在线实例不绑定并忽略。
...
CPDS在发送前必须验证：每个 `(deviceType, ESN)`只对应一个 `deviceId`，每个 `(deviceType, deviceId)`只对应一个ESN，并且类型数量与本次冻结的认证目标清单一致。数量完全匹配时该清单等于节点期望清单；用户确认继续后只包含已匹配在线设备。发现冲突时以 `AUTH_ASSIGNMENT_CONFLICT`在本地终止认证，不发送冲突记录。
...
CPDS收到某CPDC失败的 `TRANSFER_COMPLETE_RSP`后，将该设备记为传输失败终态，页面显示“已检测到设备失败，正在完成其他在线设备”，停止为该失败设备单独补包，但不得停止其他在线设备的发送、补发、解析和最终结果确认。只有本次认证目标中的全部CPDC都进入成功或失败终态后，才按数量是否原本匹配及成功设备数量结算并弹出最终结果。
...
CPDS全局阶段在本次冻结的认证目标中全部CPDC完成传输前保持 `Transferring`，但必须随时接收并处理已提前完成解析的CPDC最终结果。收到有效 `PARSE_COMPLETE_REQ`时必须立即回复 `PARSE_COMPLETE_ACK`，不得等待全局阶段切换。
...
`result=RESULT_SUCCESS`时，CPDS将该实例标记为成功终态；`result=RESULT_FAILED`时，CPDS发送ACK后将该实例标记为失败终态，并继续处理其他非终态设备，直到本次认证目标中的全部CPDC进入成功或失败终态。此后统一结算：零台成功为失败；至少一台成功但存在未发现、被忽略或失败设备时为部分成功；只有发现数量完全匹配且全部目标设备成功时为全部成功。ACK只确认CPDS收到结果，不代表CPDS把失败结果改为成功。
...
发现阶段发生ESN冲突、非法报文或数量不匹配后用户取消/无可匹配设备时，CPDS结束Distribution并弹出最终失败结果。认证阶段失败时同样立即结束。传输或解析阶段出现首个失败时，CPDS不得立即终止其他设备；应继续让本次认证目标中的所有非终态设备完成接收、自动解析和最终结果确认，全部进入终态后再按规则结算。除发现数量不匹配确认框中的“取消”外，系统不提供用户手动终止。

每次Distribution结束都必须显示最终结果弹窗：全部成功弹窗显示成功摘要；部分成功弹窗显示数量不匹配和/或设备失败详情，并分别列出成功、失败、未发现和被忽略的多出设备；失败弹窗列出失败阶段、设备类型、ESN后6位、已认证时的设备ID、统一错误码及由当前语言资源映射的文案，并分别列出成功完成、失败和掉线的全部设备。全部成功和部分成功弹窗都必须增加提示“请重启通信参数下发成功的相关设备。”。数量不匹配确认框不替代最终结果弹窗；无可匹配设备时不显示数量确认框，直接显示一次最终失败弹窗。

首次检测到传输或解析失败至最终弹窗之间，页面持续显示“已检测到设备失败，正在完成其他在线设备”。

主要失败条件：

- 发现数量不匹配后用户取消或无可匹配设备，以及ESN冲突或发现报文无效。
...
sequenceDiagram
    ...
    alt 数量不匹配且存在可匹配设备
        S-->>S: 弹窗等待用户选择继续或取消
        Note over S,C: 继续时仅匹配在线设备进入后续流程；取消时任务失败结束
    else 数量不匹配且无可匹配设备
        S-->>S: 不显示数量确认框，判定为失败
    else 数量完全匹配
        Note over S,C: 全部期望设备进入后续流程
    end
    break 用户取消或无可匹配设备
        S-->>S: 显示最终失败弹窗并结束任务
    end
...
9. 多类型CPDC任一类型解析失败时，该CPDC实例进入失败终态；其他目标至少一台成功时会话为部分成功，全部目标均失败时会话失败。
...
14. 发现阶段第0至4秒各发送一次请求，第5秒收集全部有效回复后统一判断，不因提前匹配而提前进入认证；数量不匹配时必须先等待用户确认，确认前不发送认证消息。
...
35. 传输或解析阶段出现失败后，CPDS等待本次认证目标中的所有设备进入成功或失败终态；零台成功时结算为失败，至少一台成功时结算为部分成功，并弹出对应明细。
...
49. CPDS前端接口 `activeState/failures`使用固定结构，`activeState`包含等待发现数量确认和部分成功状态；UDP协议不携带这些字段。`failures`不含自由文本原因，可携带结构化翻译参数，全部流程错误统一使用Proto `ErrorCode`枚举。
...
68. 发现设备数量少于、多于或按类型计数不一致且至少存在一台可匹配设备时，弹窗显示数量差异并提供继续/取消；继续后只向共同索引范围内的匹配在线设备下发，多出的设备不认证，最终至少一台成功即为部分成功、全部失败即为失败。
69. 数量不匹配且没有任何可匹配设备时不显示数量确认框，直接把任务标记为失败并显示一次最终失败弹窗；ESN冲突和非法发现报文始终不允许继续。
70. 每次Distribution结束均显示全部成功、部分成功或失败弹窗；全部成功和部分成功弹窗均显示“请重启通信参数下发成功的相关设备。”，发现阶段数量确认框不能替代最终结果弹窗。
